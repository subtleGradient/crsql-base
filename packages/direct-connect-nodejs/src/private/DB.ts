...
import { Change } from "@vlcn.io-community/direct-connect-common";
...
export default class DB {
  ...
  constructor(
    ...
    const siteidTableExists = this.db
      .prepare(
        "SELECT count(*) FROM sqlite_master WHERE type='table' AND name='crsql_site_id'"
      )
      .pluck()
      .get();
    if (siteidTableExists == 0) {
      this.db.exec(
        `CREATE TABLE crsql_site_id (site_id BLOB NOT NULL, ordinal INTEGER PRIMARY KEY AUTOINCREMENT)`
      );
      this.db
        .prepare(`INSERT INTO "crsql_site_id" (site_id, ordinal) VALUES (?, 0)`)
        .run(dbid);
    }

    this.db.loadExtension(extensionPath);
    this.#pullChangesetStmt = this.db.prepare(
      `SELECT "table", "pk", "cid", "val", "col_version", "db_version", "cl", "seq" FROM crsql_changes WHERE db_version > ? AND site_id IS NOT ?`
    );
